                        ...
                        {!showNotifPrefsBell ? (
                          <>
                            ...
                            <div className="px-5 pt-5 pb-4">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)' }}>
                                    <Bell className="w-4 h-4 text-red-400" />
                                  </div>
                                  <div>
                                    <p className="font-bold text-white text-[15px] leading-tight">Notifications</p>
                                    {unreadCount > 0
                                      ? <p className="text-[11px] text-red-400 font-medium mt-0.5">{unreadCount} non lue{unreadCount > 1 ? 's' : ''}</p>
                                      : <p className="text-[11px] text-white/30 mt-0.5">Tout est à jour</p>
                                    }
                                  </div>
                                </div>
                                <div className="flex items-center gap-1">
                                  {unreadCount > 0 && (
                                    <button onClick={markAllRead} title="Tout marquer lu"
                                      className="w-8 h-8 rounded-xl flex items-center justify-center text-white/30 hover:text-emerald-400 hover:bg-emerald-500/10 transition-all">
                                  ...
                                  <button onClick={() => setShowNotifications(false)}
                                    className="w-8 h-8 rounded-xl flex items-center justify-center text-white/30 hover:text-white hover:bg-white/[0.07] transition-all">
                            ...
                            <div className="overflow-y-auto" style={{ maxHeight: '340px' }}>
                              {loadingNotifs ? (
                                <div className="flex items-center justify-center py-12">
                                  <div className="w-5 h-5 border-2 border-white/10 border-t-red-500/60 rounded-full animate-spin" />
                                </div>
                              ) : notifications.length === 0 ? (
                                ...
                                <div className="py-2">
                                  {notifications.map((notif, i) => (
                                    <motion.div
                                      ...
                                      className={cn(
                                        'group flex gap-3.5 px-5 py-3.5 cursor-pointer transition-all relative',
                                        !notif.is_read ? 'bg-red-500/[0.05]' : 'hover:bg-white/[0.03]'
                                      )}
                                      onClick={async () => {
                                    ...
                                    >
                                      {/* Unread indicator */}
                                      {!notif.is_read && (
                                        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 bg-red-500 rounded-r-full" />
                                      )}

                                      {/* Poster or icon */}
                                      {notif.image_url ? (
                                        <div className="relative w-11 h-[62px] flex-shrink-0 rounded-xl overflow-hidden" style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.4)' }}>
                                        ...
                                        <div className="w-11 h-11 flex-shrink-0 rounded-xl flex items-center justify-center" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.15)' }}>
                                          <Bell className="w-4 h-4 text-red-400" />
                                        </div>
                                      )}
                                      ...
                                      <div className="flex-1 min-w-0 py-0.5">
                                        <p className={cn('text-[13px] font-semibold mb-1 leading-tight', notif.is_read ? 'text-white/70' : 'text-white')}>
                                          {notif.title}
                                        </p>
                                        <p className="text-white/40 text-[12px] line-clamp-2 leading-relaxed">{notif.message}</p>
                                        <p className="text-white/20 text-[11px] mt-1.5 font-medium">{timeAgo(notif.created_at)}</p>
                                      </div>

                                      {!notif.is_read && <div className="w-2 h-2 rounded-full bg-red-500 mt-2 flex-shrink-0 shadow-[0_0_6px_rgba(239,68,68,0.6)]" />}
                                    </motion.div>
                                  ))}
                                </div>
                              )}
                            ...
                            <button
                              ...
                              className="w-full flex items-center justify-center gap-2 py-3.5 text-white/30 hover:text-white/60 text-xs font-medium hover:bg-white/[0.03] transition-all"
                            >
                          ...
                          <>
                            ...
                            <div className="p-4 space-y-2 max-h-80 overflow-y-auto">
                              {prefConfig.map(pref => {
                                ...
                                return (
                                  <div key={pref.key} className="flex items-center gap-3 p-3.5 rounded-xl transition-all" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
