...
export function ContentGrid({ title, content, type }: ContentGridProps) {
  ...
  useEffect(() => { setPage(1) }, [search, sort, selectedGenre])
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-white/30 text-sm ml-4">
            {displayed.length} / {filtered.length} {type === 'movie' ? 'titre' : 'série'}{filtered.length > 1 ? 's' : ''}
          </p>
        </div>
      ...
      {view === 'grid' && (
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3">
          {displayed.map((item) => {
            ...
            return (
              <TiltCard key={item.id} onClick={() => openDrawer(type, tmdbId)}>
                <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-zinc-800 mb-2">
                  <Image src={getPosterUrl(item.poster_path)} alt={t} fill className="object-cover transition-transform duration-300" sizes="14vw" />
      ...
      {view === 'list' && (
        <div className="space-y-2">
          {displayed.map((item) => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-4 p-3 bg-white/5 hover:bg-white/8 border border-white/5 hover:border-white/15 rounded-xl transition-all group cursor-pointer" onClick={() => openDrawer(type, tmdbId)}>
