...
import { Search, LayoutGrid, List, SlidersHorizontal, X, ChevronRight } from 'lucide-react'
...
export function ContentGrid({ title, content, type }: ContentGridProps) {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      case 'rating': items.sort((a, b) => (b.vote_average || 0) - (a.vote_average || 0)); break
      case 'newest': items.sort((a, b) => { const da = isMovie(a) ? a.release_date : (a as Series).first_air_date; const db = isMovie(b) ? b.release_date : (b as Series).first_air_date; return new Date(db || 0).getTime() - new Date(da || 0).getTime() }); break
      case 'recent': items.sort((a, b) => (b.id || 0) - (a.id || 0)); break
      case 'alpha': items.sort((a, b) => { const ta = isMovie(a) ? a.title : (a as Series).name; const tb = isMovie(b) ? b.title : (b as Series).name; return ta.localeCompare(tb) }); break
    }
    return items
    ...
    <div className="container mx-auto px-6 py-8">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1">
          <button onClick={() => setView('grid')} className={`p-2 rounded-lg transition-all ${view === 'grid' ? 'bg-white/15 text-white' : 'text-white/30 hover:text-white/60'}`}>
            <LayoutGrid className="w-4 h-4" />
          </button>
          <button onClick={() => setView('list')} className={`p-2 rounded-lg transition-all ${view === 'list' ? 'bg-white/15 text-white' : 'text-white/30 hover:text-white/60'}`}>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
        <div className="flex-1 flex items-center gap-3 bg-zinc-900 border border-white/10 rounded-2xl px-4 py-3">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Rechercher un${type === 'movie' ? ' film' : 'e série'}...`} className="flex-1 bg-transparent text-white text-sm outline-none placeholder-white/20" />
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-0.5">
          {(Object.keys(sortLabels) as SortType[]).map(s => (
            <button key={s} onClick={() => setSort(s)} className={`flex-shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${sort === s ? 'bg-white/15 text-white border border-white/20' : 'text-white/40 hover:text-white/70'}`}>
              {sortLabels[s]}
            </button>
          ))}
          <button onClick={() => setShowFilters(!showFilters)} className={`flex-shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${showFilters || selectedGenre ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-white/5 text-white/40 hover:text-white/70 border border-white/10'}`}>
        ...
        {showFilters && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-6">
            <div className="flex flex-wrap gap-2 py-2">
              <button onClick={() => setSelectedGenre(null)} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${!selectedGenre ? 'bg-primary text-white' : 'bg-white/10 text-white/50 hover:bg-white/15'}`}>Tous</button>
              {allGenres.map(([id, name]) => (
                <button key={id} onClick={() => setSelectedGenre(selectedGenre === +id ? null : +id)} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${selectedGenre === +id ? 'bg-primary text-white' : 'bg-white/10 text-white/50 hover:bg-white/15'}`}>{name}</button>
              ))}
            </div>
          </motion.div>
        )}
          ...
          {visible.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                transition={{ duration: 0.3, delay: (i % PAGE_SIZE) * 0.02, ease: 'easeOut' }}
              >
          ...
          {visible.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                transition={{ duration: 0.25, delay: (i % PAGE_SIZE) * 0.015, ease: 'easeOut' }}
              >
                <div className="flex items-center gap-4 p-3 bg-white/5 hover:bg-white/8 border border-white/5 hover:border-white/15 rounded-xl transition-all group cursor-pointer" onClick={() => openDrawer(type, tmdbId)}>
                  ...
                  <div className="flex items-center gap-1 text-yellow-400 text-sm font-bold flex-shrink-0">★ {item.vote_average?.toFixed(1)}</div>
      ...
      <div ref={loaderRef} className="flex justify-center py-8">
        {hasMore && (
          <motion.div animate={{ opacity: [0.3, 1, 0.3] }} transition={{ repeat: Infinity, duration: 1.2 }} className="flex items-center gap-2 text-white/30 text-sm">
